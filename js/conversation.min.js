(function () {
    ...
    var B = d.append("g").attr("class", "links"),
        f = d.append("g").attr("class", "tumors"),
        E = d.append("g").attr("class", "nodes");
    var Q = d3.select("#graph-info");
    d3.text("makeRequest.php?dataset=pubmed", function (X, dataStr) {
    	var dataParts = dataStr.split('<body>');
  		var data = JSON.parse(dataParts[dataParts.length-1].split('</body>')[0]);
  		
		var tumors = [], publications = [];
		var tumorLocator = [];
		console.log("here");
		var count = 0;
		for (i in data.results.bindings) {
			var binding = data.results.bindings[i];
			var tumorID = binding["o"].value.split(/[:#\/]/);
			
			var identifier = binding["s"].value.split(/[:#\/]/);
			if(tumorLocator[tumorID] == null) {
				var newTumor = {"type": "tumor", "name" : tumorID[tumorID.length-1], "description": "", "tumor": count+1, "count": 1, "slug": binding["o"].value, "links": []};
				var newPage = {"type": "publication", "name": identifier[identifier.length-1], "description": binding["title"].value, "slug":  binding["s"].value};
				newTumor.links.push(identifier[identifier.length-1]);
				publications.push(newPage);
				tumors.push(newTumor);
				tumorLocator[tumorID] = count++;
			} else {
				var tumor = tumors[tumorLocator[tumorID]];
				var newPage = {"type": "publication", "name": identifier[identifier.length-1], "description": binding["title"].value, "slug":  binding["s"].value};
				tumor.links.push(identifier[identifier.length-1]);
				publications.push(newPage);
				tumor.count++;
			}
		}
		var Y = {"tumors" : tumors, "publications" : publications, "perspectives" : []};
		console.log(Y);
        T = d3.map(Y);
        q = d3.merge(T.values());
        x = {};
        ...
        j = d3.map();
        T.get("tumors").forEach(function (aa) {
    ...
    function O() {
        ...
        i = Math.floor(c / T.get("tumors").length);
        y = Math.floor(T.get("tumors").length * i / 2);
        T.get("tumors").forEach(function (af, ae) {
            af.x = U / -2;
            af.y = ae * i - y
        });
        var ad = J,
            Z = 360 - J,
            ac = (Z - ad) / (T.get("publications").length - 1);
        T.get("publications").forEach(function (af, ae) {
            af.x = Z - ae * ac;
            af.y = h / 2 - R;
            af.xOffset = -S;
            af.depth = 1
      ...
      /*  ad = J;
        ...
        H = [];
        var ab, Y, aa, X = h / 2 - R;
        T.get("tumors").forEach(function (ae) {
            ae.links.forEach(function (af) {
                ...
                H.push({
                    ...
                    key: aa,
                    canonicalKey: aa,
                    x1: ae.x + (ab.type === "publication" ? 0 : U),
                    y1: ae.y + K / 2,
    ...
    function M() {
        ...
        D(T.get("tumors"));
        b(d3.merge([T.get("publications"), T.get("perspectives")]));
        C([]);
        m(P);
    ...
    function C(Z) {
        ...
        if (ab && ab.type === "tumor") {
            ...
            aa.append("text").attr("fill", N).attr("text-anchor", "middle").attr("y", (o + t) * -1).text(function (ae) {
                return "Tumor " + ae.tumor
            })
        } else {
            if (ab && ab.type === "publication") {
                Y.append("text").attr("fill", "#aaa").attr("text-anchor", "middle").attr("y", (o + t) * -1).text("Publication")
            } else {
        ...
        ac.exit().remove();
        var X = d.selectAll(".all-tumors").data(Z);
        X.enter().append("text").attr("text-anchor", "start").attr("x", a / -2 + t).attr("y", c / 2 - t).text("all tumors").attr("class", "all-tumors").on("click", O);
        X.exit().remove()
    }

    function D(Y) {
        var Y = f.selectAll(".tumor").data(Y, u);
        var X = Y.enter().append("g").attr("class", "tumor").on("mouseover", g).on("mouseout", n).on("click", G);
    ...
    function z() {
        ...
        E.selectAll("circle").attr("fill", function (X) {
            ...
            if (X === L.node) {
                ...
                if (X.type === "publication" || X.type === "perspective") {
                    return 1.5
                }
            }
            return null
